...
export const  Profile=() => {
    ...
    const [name,setName]=useState("")
      ...
      useEffect(() => {
        Axios({
            ...
            console.log(response);
            setPredictions(response.data.queries)
          }, (error) => {
            console.log(error);
          });
      },[])
  ...
  const displayDesktop = (name) => {
    return <Toolbar>Profile {name}</Toolbar>;
  }

  return (
      <div>
            <h1 className="hm" class="p-3 mb-2 bg-info text-white">Profile</h1>
    <TableContainer component={Paper}>
      <Table className={classes.table} aria-label="custom pagination table">
      <TableHead>
            <TableRow>
              <TableCell style={{fontSize:20 }}>Queries</TableCell>
            </TableRow>
          </TableHead>
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={prediction.queries}>
